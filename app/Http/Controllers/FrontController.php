...
class FrontController extends Controller
{
    ...
    public function allProducts(Request $request)
    {
        ...
        $minRange=($request->range) ? explode(',', $request->range)[0] : null;
        $maxRange=($request->range) ? explode(',', $request->range)[1] : null;
        $min = ShopProduct::with('category', 'rating')->min('price');
        $max = ShopProduct::with('category', 'rating')->max('price');
        
        return view('all-products', compact('products', 'min', 'max','minRange','maxRange'));
    }
